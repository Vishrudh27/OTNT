// // wireguard.js - Cross-platform WireGuard helper (final updated)
...
//   // also check live tunnels map if passed
//   if (tunnelsMap) {
...
// // --- Cleanup stale tunnels on boot or when needed ---
...
//   try {
//     await exec(`ip link show ${ifaceName}`); // exists if no error
//   } catch {
...
// // NOTE: accepts `address` (serverIP). Falls back to getNextFreeIP() if omitted.
...
// // --- Serialize tunnel creation to avoid IP races ---
// let tunnelCreationLock = Promise.resolve();

// // --- Create tunnel ---
// async function createTunnel({ privateKey, address, peerPublicKey, allowedIPs, endpoint }) {
//   return (tunnelCreationLock = tunnelCreationLock.then(async () => {
//     const ifaceName = `wg${Math.floor(Math.random() * 10000)}`;
//     const confDir = getConfigDir();
...
//       // 🧹 ensure iface is cleaned before reuse
